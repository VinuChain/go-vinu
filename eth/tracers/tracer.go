...
const (
	// maxTracerCallDepth caps the call frame nesting depth for the JS tracer
	// to prevent Duktape C stack overflow. The EVM itself allows up to 1024.
	maxTracerCallDepth = 512
)
...
type Tracer struct {
	...
	callFrameDepth    int              // Current call frame nesting depth
}
...
func (jst *Tracer) CaptureExit(output []byte, gasUsed uint64, err error) {
	...
	// Decrement before the error check: the EVM guarantees every CaptureEnter
	// is paired with a CaptureExit via defer, even when CaptureEnter set an
	// error (e.g. depth limit). Decrementing first keeps the counter balanced.
	jst.callFrameDepth--
	if jst.err != nil {
		return
	}
